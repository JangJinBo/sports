...
<div class="frame jusel" >
	<div>
		<div class="miniNav">
			...
			<h2 class="listselected"><a href="${cpath }/juBoard/matchBoard">팀 매칭</a></h2>
		</div>
		<div id="trainingSearch" class="juselSearch">
			<form method="GET" action="${cpath }/juBoard/matchBoard">
	...
	<table>
		...
		<tbody>
			<c:forEach var="mtDto" items="${mtBoardList }">
			<tr class="toJuView" onclick="select('${mtDto.idx }')">
				<td class="cen">${mtDto.idx }</td>
				<td class="cen">${mtDto.category }</td>
				<td>${mtDto.title }</td>
				<td class="cen relbox" onContextMenu="right('userid')">${mtDto.username}
				<div class="memberbox mb${mtDto.userid } disnone mb">
					<div class="mb"><a href="${cpath }/message/sendMessage?receiverId=${mtDto.userid}" class="mb">✉ 쪽지보내기</a></div>
				</div>
				</td>
				<td class="cen">${mtDto.wdate }</td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
...
<script>
	function select(sel) {
		location.href = cpath + '/juBoard/mtchView/' + sel + '?pageNum=${pageMake.pageNum}&searchType=${searchType}&keyword=${keyword}'
	}
