...
<link type="text/css" rel="stylesheet" 
	...
	<div class="frame listHead">
		...
		<form method="POST" enctype="multipart/form-data" id="regForm">
				<input type="hidden" name="username" value="${ptydto.username }" required>
				<input type="hidden" name="juBoard_idx" value="${ptydto.juBoard_idx }" required>
				<input type="hidden" name="board_idx" value="${ptydto.board_idx }" required>
		<table id="regtb">
		<tr>
		<td>제목</td>
			<td>
				<input type="text" name="title"
					placeholder="제목 입력" value="${ptydto.title }" required autofocus>
			</td>
		</tr>
		<tr>
			<td>나이</td>
			<td>
				<input type="text" name="age"
					placeholder="나이 입력" value="${ptydto.age }" required autofocus>
			</td>
		</tr>
		<tr>
			<td>성별</td>
		...
		<tr>
			...
			<td>
				<textarea name="content" placeholder="팀 소개"
					style="resize: none; width: 500px; height: 200px">${ptydto.content }</textarea>
			</td>
		</tr>
		<tr>
			<td>종목</td>
			...
			<input type="text"
				...
				placeholder="종목 입력" value="${ptydto.category }" required autofocus>
			</td>
		</tr>
		<tr>
			<td>지역명</td>
			...
			<input type="text"
				...
				placeholder="간단한 지역명 입력" value="${ptydto.mainarea }" required autofocus>
			</td>
		</tr>
		<tr>
			<td>정확한 장소</td>
			<td>
				<input type="text" name="place" placeholder="정확한 장소 입력" value="${ptydto.place }">
			</td>
		</tr>
		<tr>
			<td>일정 입력</td>
			<td>
				<input type="date" name="schedule" value="${ptydto.schedule }">
			</td>
		</tr>
		<tr>
			<td>인원수</td>
			<td>
				<input type="number" value="${ptydto.maxCapacity }" name="maxCapacity" placeholder="인원수 입력">
			</td>
		</tr>
		</table>
	...
	<script>
		const ptyCancel = document.getElementById('ptyCancel')
		ptyCancel.onclick = function ptyCancelHandler(event) {
			window.history.go(-1)
		}
	</script>
