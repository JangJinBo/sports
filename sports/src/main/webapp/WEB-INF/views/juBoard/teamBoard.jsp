...
<div class="frame jusel" >
	<div>
		<div class="miniNav">
			...
			<h2 class="listselected"><a href="${cpath }/juBoard/teamBoard">팀원 모집</a></h2>
		...
		<div id="trainingSearch" class="juselSearch">
			<form method="GET" action="${cpath }/juBoard/teamBoard">
	...
	<table>
		...
		<tbody>
			<c:forEach var="tmDto" items="${tmBoardList }">
			<tr class="toJuView" onclick="select('${tmDto.idx }')">
				<td class="cen">${tmDto.idx }</td>
				<td class="cen">${tmDto.category }</td>
				<td>${tmDto.title }</td>
				<td class="cen relbox" onContextMenu="right('userid')">${tmDto.username}
...
<script>
	function select(sel) {
		location.href = cpath + '/juBoard/tmView/' + sel + '?pageNum=${pageMake.pageNum}&searchType=${searchType}&keyword=${keyword}'
	}
