...
<div class="frame jusel">
	<div>
		<div class="miniNav">
			<h2 class="listselected"><a href="${cpath }/juBoard/substiBoard">용병 모집</a></h2>
			<h2 class="listnother"><a href="${cpath }/juBoard/teamBoard">팀원 모집</a></h2>
			<h2 class="listnother"><a href="${cpath }/juBoard/partyBoard">파티원 모집</a></h2>
			<h2 class="listnother"><a href="${cpath }/juBoard/matchBoard">팀 매칭</a></h2>
		</div>
		<div id="trainingSearch" class="juselSearch">
			<form method="GET" action="${cpath }/juBoard/substiBoard">
	...
	<table>
		...
		<tbody>
			<c:forEach var="sbDto" items="${sbBoardList }">
			<tr class="toJuView" onclick="select('${sbDto.idx }')">
				<td class="cen">${sbDto.idx }</td>
				<td class="cen">${sbDto.category }</td>
				<td>${sbDto.title }</td>
				<td class="cen relbox" onContextMenu="right('userid')">${sbDto.username}
				<div class="memberbox mb${sbDto.userid } disnone mb">
					<div class="mb"><a href="${cpath }/message/sendMessage?receiverId=${sbDto.userid}" class="mb">✉ 쪽지보내기</a></div>
				</div>
				</td>
				<td class="cen">${sbDto.wdate }</td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
...
<script>
	function select(sel) {
		location.href = cpath + '/juBoard/sbtiView/' + sel + '?pageNum=${pageMake.pageNum}&searchType=${searchType}&keyword=${keyword}'
	}
