...
<div class="frame event" id="training">

   <div>
      <div class="miniNav">
         <h2 class="listnother"><a href="${cpath }/event/training">강습 안내</a></h2>
         <h2 class="listselected"><a href="${cpath }/event/competition">대회 공지 안내</a></h2>
      </div>
      <div id="trainingSearch" class="eventSearch">
         <form method="GET" action="${cpath }/event/competition">
      ...
      <tbody>
         <c:forEach var="dto" items="${comList }" >
			<tr class="toEventView" onclick="select('${dto.idx }')">
				...
				<td>${dto.title }</td>
				<td class="cen relbox" onContextMenu="right('userid')">${dto.userid}
...
<script>
   function select(sel) {
      location.href = cpath + '/event/comview/' + sel + '?pageNum=${pageMake.pageNum}&searchType=${searchType }&keyword=${keyword}'
   }
