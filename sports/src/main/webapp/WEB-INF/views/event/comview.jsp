...
<div class="frame listHead eventView">
	<div>
		<h2>대회 공지</h2>
		<a href="${cpath }/event/competition?pageNum=${pageNum}&searchType=${searchType}&keyword=${keyword}"><button>≡ 목록으로</button></a>
	</div>

	<div class="viewHeader">
		<h3>대회 공지 | ${eventDTO.title }</h3>
	...
	<div class="fine">
		...
		<p class="eventviewMenu">
			...
			<a href="${cpath }/event/comDelete/${eventDTO.idx}"><button id="deleteBtn">삭제</button></a>	
		</p>
		</c:if>
	</div>
</div>
	...
	deleteBtn.onclick = function deleteBtnHandler(event) {
		...
		if ('${login.userid}' == '${eventDTO.userid }' || ${ login.memberType == 'admin'}) {
			if (confirm('삭제하시겠습니까?') == true) {
				window.location.href = cpath + '/event/comDelete/' + idx
			}
		} else {
			history.go(-1);
		}
